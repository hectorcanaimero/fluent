...
| Los 8 modelos `nvidia/*` de la lista interna de 9router responden 410 (fin de vida) o 404, pero 9router reenvía cualquier id `nvidia/<publisher>/<modelo>` del catálogo actual de NIM: `nvidia/nvidia/nemotron-3-super-120b-a12b` y `nvidia/mistralai/mistral-nemotron` responden JSON válido con `reasoning_effort: none`. `openrouter/typesafe/jev-1.13` es un modelo de "decisions", no de chat. | Los ids muertos fuera; los dos vivos entran en `fluent-free`. En OpenRouter hay que añadir a mano los `:free` en el panel de 9router. |
...
| Composición decidida por el operador el 2026-09-24. `fluent-free` = `nvidia/nvidia/nemotron-3-super-120b-a12b` → `nvidia/mistralai/mistral-nemotron` → `cf/@cf/mistralai/mistral-small-3.1-24b-instruct` → `cf/@cf/meta/llama-3.3-70b-instruct-fp8-fast`, todos con `reasoning_effort: none` (Nemotron 1–3 s, Mistral 2 s, JSON válido). `fluent-pro` = `ds/deepseek-v4-flash` → `gemini/gemini-3.5-flash-lite`, también `none` (1–2 s, con y sin streaming). Fuera de los combos: `openai/gpt-oss-120b` vía Groq, porque rechaza `none` con 400 y 9router no salta al siguiente modelo ante un 400. | `REASONING_EFFORT`: `fluent-free` → `none`, `fluent-pro` → `none`. El bench de F1.6 revalida cuando el operador lo autorice. |
