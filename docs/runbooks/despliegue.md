...
3. **Comando de arranque sobrescrito**: en la pestaña de configuración de la aplicación, busca "Custom Start Command" / "Start Command" y ponlo a `node dist/worker.js` (en vez de dejar el `CMD` por defecto de la imagen, que es `node dist/main.js`, la API). Ver `apps/api/Dockerfile` y `apps/api/src/worker.ts`.
