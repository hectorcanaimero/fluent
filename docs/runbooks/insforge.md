...
## 4. Rotar `CREDENTIALS_MASTER_KEY`

La clave maestra `CREDENTIALS_MASTER_KEY` cifra las credenciales de los proveedores de LLM (OpenRouter, Gemini) almacenadas en `provider_credentials` tabla. Se rota ocasionalmente por seguridad.

### Distinción importante

**NO confundas con `INSFORGE_API_KEY` / `INSFORGE_ANON_KEY`:**
- `CREDENTIALS_MASTER_KEY` — Secreto **de la API de Fluent**, no de InsForge. Vive en Coolify. Cifra credenciales de **proveedores LLM externos** (OpenRouter, Gemini).
- `INSFORGE_API_KEY` / `INSFORGE_ANON_KEY` — Credenciales **del proyecto InsForge mismo**. Viven en Coolify. Acceden a la BD/storage de Fluent.

La rotación de `INSFORGE_API_KEY` es un comando insforge-cli distinto (`secrets rotate api-key`); no es parte de este runbook.

### 4.1 Flujo de rotación de `CREDENTIALS_MASTER_KEY`

**Paso 1: Generar clave nueva**

En el VPS o localmente:
```bash
...
**Paso 2: Actualizar variables en Coolify**

En el panel de Coolify (`https://<coolify-url>`):
1. Navega a **Proyecto `fluent` → Entorno `production` → Aplicación `fluent-api` → Variables de Entorno**.
2. Busca `CREDENTIALS_MASTER_KEY`.
3. Copia su valor actual a un lugar seguro temporalmente.
4. Crea una **nueva variable** `CREDENTIALS_MASTER_KEY_PREVIOUS` con el valor antiguo de `CREDENTIALS_MASTER_KEY`.
5. Actualiza `CREDENTIALS_MASTER_KEY` con el valor nuevo generado.

Después del cambio, las variables en Coolify se ven así:
```
CREDENTIALS_MASTER_KEY=aB1cD2eF3gH4iJ5kL6mN7oP8qR9sT0uV1wX2yZ3aB4cD=
CREDENTIALS_MASTER_KEY_PREVIOUS=<valor anterior>
```

**Paso 3: Redeploy de la API**

Coolify dispara un redeploy automático al cambiar variables (o puedes hacerlo manualmente). La API arranca con:
- `CREDENTIALS_MASTER_KEY` = clave nueva
- `CREDENTIALS_MASTER_KEY_PREVIOUS` = clave antigua (fallback)

**Paso 4: Verificar descifrado de credenciales existentes**

Haz una llamada de prueba desde la app o curl para asegurar que el descifrado aún funciona:
```bash
curl -X GET https://fluent-api.13.140.175.146.sslip.io/v1/me \
  -H "Authorization: Bearer <token-usuario>"
```

La app debería cargar datos del usuario sin error. Internamente, la API intenta descifrar las credenciales existentes:
1. Primero con `CREDENTIALS_MASTER_KEY` (clave nueva).
2. Si falla, intenta con `CREDENTIALS_MASTER_KEY_PREVIOUS` (clave antigua).

**Paso 5: Remover variable anterior (opcional, después de días)**

Después de días o semanas en producción (sin errores de descifrado):
1. En Coolify, elimina la variable `CREDENTIALS_MASTER_KEY_PREVIOUS`.
2. Redeploy.

Esto es un paso de limpieza; no es urgente si no hay errores.

### 4.2 Rotación de `INSFORGE_API_KEY` / `INSFORGE_ANON_KEY` (distinto)

Si necesitas rotar las credenciales **del proyecto InsForge** (no la clave maestra de cifrado de Fluent):

```bash
npx -y @insforge/cli secrets rotate api-key [--grace-hours 24]
npx -y @insforge/cli secrets rotate anon-key [--grace-hours 24]
```

Opciones:
- `api-key` — Rota la clave admin de InsForge (alto impacto).
- `anon-key` — Rota la clave anónima de InsForge (usada por la app móvil).
- `--grace-hours <n>` — Tiempo que la clave antigua sigue funcionando después de la rotación (por defecto el servidor decide). 24 horas es típico.

**Después de rotar:**
1. El comando imprime la **nueva clave una sola vez** — cópiala inmediatamente.
2. Actualiza la variable correspondiente en Coolify (`INSFORGE_API_KEY` o `INSFORGE_ANON_KEY`).
3. Redeploy antes de que expire el grace period.

Salida (ejemplo):
```
✓ Rotated. Old key expires in 24 hours.
New API key: ik_live_xxxxx...

Update your env vars before: 2026-09-09T15:30:00Z
```
